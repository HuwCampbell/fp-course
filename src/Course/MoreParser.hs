...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok open close =
  between (charTok open) (charTok close)
...
hex ::
  Parser Char
hex = do
  raw <- replicateA 4 (satisfy isHexDigit)
  case readHex raw of
    Full a -> pure (chr a)
    Empty -> P $ \_ -> ParseError (UnexpectedString raw)
...
sepby1 pa sep = do
  a    <- pa
  rest <- (sep *> sepby1 pa sep) ||| pure Nil
  pure (a  :. rest)
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma open close inner =
  betweenCharTok open close (sepby (tok inner) (charTok ','))
